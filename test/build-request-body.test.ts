...
import { buildRequestBody, mergeDeep, type PolyDocParams } from '../src/lib/build-request-body.js';
...
describe('buildRequestBody', () => {
  ...
  it('adds screenshot.encoding when base64 is requested', () => {
    const r = buildRequestBody(
      base({ operation: 'screenshot', screenshotOptions: { encoding: 'base64' } }),
    );
    expect((r.body.screenshot as Record<string, unknown>).encoding).toBe('base64');
  });
  ...
  it('Save to Directus File is treated as binary delivery', () => {
    const r = buildRequestBody(base({ delivery: { mode: 'directusFile' } }));
    expect(r.isBinary).toBe(true);
    expect(r.body).not.toHaveProperty('cloudStorage');
    expect(r.body).not.toHaveProperty('webhook');
  });

  it('Base64 delivery is binary and adds no delivery field to the body', () => {
    const r = buildRequestBody(base({ delivery: { mode: 'base64' } }));
    expect(r.isBinary).toBe(true);
    expect(r.body).not.toHaveProperty('cloudStorage');
    expect(r.body).not.toHaveProperty('webhook');
  });
